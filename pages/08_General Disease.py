...
st.write("Choose any symptoms that you are experiencing based on the category and we will do the best to diagnose it")

skin = ['itching', 'skin_rash', 'nodal_skin_eruptions', 'cold_hands_and_feets', 'yellowish_skin', 'puffy_face_and_eyes',
        'brittle_nails', 'swollen_extremeties', 'blister', 'inflammatory_nails', 'small_dents_in_nails', 'skin_peeling',
        'pus_filled_pimples', 'blackheads', 'scurring']

digestion = ['stomach_pain', 'acidity', 'ulcers_on_tongue', 'vomiting''weight_gain', 'patches_in_throat',
             'irregular_sugar_level', 'indigestion', 'loss_of_appetite', 'constipation', 'abdominal_pain', 'diarrhoea',
             'swelling_of_stomach''fluid_overload', 'pain_during_bowel_movements', 'pain_in_anal_region',
             'bloody_stool', 'stomach_bleeding', 'irritation_in_anus', 'cramps', 'passage_of_gases', 'belly_pain', ]
...
for i in range(len(skin)):
    ...
    skin[i] = skin[i].title()
st.subheader("Skin")
skinselect = st.multiselect("Choose anny symptoms that is related to the integumentary system", skin)
for i in range(len(digestion)):
    digestion[i] = digestion[i].replace("_", " ")
    digestion[i] = digestion[i].title()
st.subheader("Digestive System")
digestionselect = st.multiselect("Choose any symptoms that are related to the digestive system", digestion)
st.subheader("Other")
